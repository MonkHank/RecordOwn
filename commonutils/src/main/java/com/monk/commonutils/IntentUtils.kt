...
object IntentUtils {
    ...
    const val PHOTO_REQUEST_GALLERY = 0x01
    const val PHOTO_REQUEST_CAMERA = 0x02
    const val PHOTO_REQUEST_CUT = 0x03
    const val GO_TO_LOCAL_CONTACT = 1709211424

    /**
     * 跳转至外部浏览器
     */
    fun skipBrowser(context: Context, url: String?) {
        val intent = Intent()
     ...
     * 跳转至外部打电话
     */
    fun skipCallUp(context: Context, phoneNumber: String) {
...
//            request.requestPermission(Manifest.permission.CALL_PHONE);
     ...
     * 打电话
     */
    fun skipCallUp2(context: Context, phoneNumber: String) {
     ...
     * 跳转至发送短信
     */
    fun skipSendMessage(context: Context, phone: String, message: String?) {
     ...
     * 跳转至系统相册
     */
    fun skipGallery(activity: Activity, v4Fragment: Fragment?) {
     ...
     */
    fun skipContact(activity: Activity) {
        try {
...
//            LogUtil.setLog(activity, e.toString());
     ...
     * @param context
     */
    fun skipToAppListSettings(context: Context) {
        val intent = Intent(Settings.ACTION_LOCATION_SOURCE_SETTINGS)
        context.startActivity(intent)
    }

    // 针对此应用跳转至录屏界面
    private val Permissions_Video = arrayOf(
